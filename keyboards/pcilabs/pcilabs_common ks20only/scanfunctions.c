    ...
    for (uint8_t row = 0; row < MATRIX_ROWS; row++) {
        for (uint8_t col = 0; col < MATRIX_COLS; col++) {
            ...
            key->offset                   = rest_adc_value - key->raw;
             if (key->offset < -200) {
                key->switch_type = latenpow; // Assume `latenpow` is an enum or macro defined elsewhere
            } else if (key->offset > -150) {
                key->switch_type = KS_20; // Assume `KS_20` is an enum or macro defined elsewhere
            }
            key->lut = &luts[key->switch_type].values;
        }
    }
