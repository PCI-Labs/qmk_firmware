...
void write_firmware_version(void) {
    for (size_t i = 0; i < sizeof(FIRMWARE_VERSION); i++) {
        eeprom_write_byte((uint8_t *)FIRMWARE_VERSION_EEPROM_OFFSET + i, FIRMWARE_VERSION[i]);
    }
}

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {[0] = LAYOUT(KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO)};
...
void config_init_user(void) {
    config_update_user(default_config.raw);
}
...
void config_read_user(void) {
    ...
    // user_config.rgb_val);
    rgb_matrix_enable();
    rgb_matrix_sethsv(user_config.rgb_color.h, user_config.rgb_color.s, user_config.rgb_color.v);
...
void raw_hid_receive(uint8_t *data, uint8_t length) {
    switch (data[0]) {
        case 1:;
            ...
            rgb_matrix_sethsv_noeeprom(user_config.rgb_color.h, user_config.rgb_color.s, user_config.rgb_color.v);
            rgb_matrix_mode_noeeprom(user_config.rgb_mode);
            rgb_matrix_set_speed_noeeprom(user_config.rgb_speed);
...
void read_key() {
    ...
    */
    /*
    #if SENSOR_REVERSE_POLARITY == TRUE
        uint16_t value = constrain(raw, user_config_ptr->rest_positions[key_index], user_config_ptr->down_positions[key_index]);
        value          = KEY_TRAVEL_DISTANCE - map(value, user_config_ptr->rest_positions[key_index], user_config_ptr->down_positions[key_index], 0, KEY_TRAVEL_DISTANCE);
    #else
        uint16_t value = constrain(raw, user_config_ptr->down_positions[key_index], user_config_ptr->rest_positions[key_index]);
        value          = map(value, user_config_ptr->down_positions[key_index], user_config_ptr->rest_positions[key_index], 0, KEY_TRAVEL_DISTANCE);
    #endif
    */

    uint16_t value = adc_to_distance(raw);
    value          = 255 - value;

#ifdef DEBUG
    if (timer_elapsed(timer_debug[key_index]) > 1300) {
        /*
...
void handle_traditional(uint16_t value) {
    if (value <= KEY_TRAVEL_DISTANCE - user_config_ptr->actuation_point[key_index])
        press_key();
    else
        release_key();
}
...
void handle_RT(uint16_t value) {
    ...
    if (value <= KEY_TRAVEL_DISTANCE - user_config_ptr->actuation_point[key_index] && !currentKeyStates[key_index]->inZone) {
        press_key();
        currentKeyStates[key_index]->inZone = true;
    }

    else if (!currentKeyStates[key_index]->pressed && currentKeyStates[key_index]->inZone && value + user_config_ptr->rapidTriggerSensivityDown[key_index] <= currentKeyStates[key_index]->peak) {
        press_key();
    } else if (currentKeyStates[key_index]->pressed && (!currentKeyStates[key_index]->inZone || value >= currentKeyStates[key_index]->peak + user_config_ptr->rapidTriggerSensivityUp[key_index])) {
        release_key();
    }
    if ((currentKeyStates[key_index]->pressed && value < currentKeyStates[key_index]->peak) || (!currentKeyStates[key_index]->pressed && value > currentKeyStates[key_index]->peak)) {
        currentKeyStates[key_index]->peak = value;
    }
}

void handle_CRT(uint16_t value) {
    if (value >= KEY_TRAVEL_DISTANCE - 10 || value <= user_config_ptr->dead_zone[key_index]) {
        currentKeyStates[key_index]->inZone = false;
    }
...
void press_key() {
    if (currentKeyStates[key_index]->pressed) return;
...
#if SENSOR_REVERSE_POLARITY == TRUE
            calibration_rest_positions[key_index] += analogReadPin(pins[key_index]) + 5;
#else
            calibration_rest_positions[key_index] += analogReadPin(pins[key_index]) - 5;
#endif
        }
        rest_positions_avg[key_index] = calibration_rest_positions[key_index] >> 7;
        // positions_difference[key_index] = (rest_positions_avg[key_index] + REST_POS_MAGIC) - (user_config_ptr->down_positions[key_index] + DOWN_POS_MAGIC);
    }
    memcpy(user_config_ptr->rest_positions, rest_positions_avg, sizeof(rest_positions_avg));
}
/*
void calibration_down(void) {
...
uint16_t SMA_filter(uint16_t value) {
    sum_SMA[key_index]                       = sum_SMA[key_index] - buf_SMA[key_index][index_SMA[key_index]] + value;
    buf_SMA[key_index][index_SMA[key_index]] = value;
    index_SMA[key_index]++;
    if (index_SMA[key_index] == SMA_SAMPLE_SIZE) index_SMA[key_index] = 0;
...
void autocalibration_init(void) {
    ...
    for (size_t i = 0; i < KEY_NUM; i++) {
        base_difference[i]                 = REST_CALIBRATION_BASE - user_config_ptr->rest_positions[i];
        user_config_ptr->down_positions[i] = DOWN_CALIBRATION_BASE - base_difference[i];
...
void autocalibration(uint16_t value) {
    sum_SMA_down[key_index]                            = sum_SMA_down[key_index] - buf_SMA_down[key_index][index_SMA_down[key_index]] + value - 3;
    buf_SMA_down[key_index][index_SMA_down[key_index]] = value - 3;
    index_SMA_down[key_index]++;
    if (index_SMA_down[key_index] == AUTOCALIBRATION_SAMPLE_SIZE) index_SMA_down[key_index] = 0;
    user_config_ptr->down_positions[key_index] = (sum_SMA_down[key_index] >> AUTOCALIBRATION_SAMPLE_EXP);
}
#    else
void autocalibration_init(void) {
    int32_t base_difference[KEY_NUM];
    for (size_t i = 0; i < KEY_NUM; i++) {
        base_difference[i]                 = user_config_ptr->rest_positions[i] - REST_CALIBRATION_BASE;
        user_config_ptr->down_positions[i] = DOWN_CALIBRATION_BASE + base_difference[i];
        for (size_t j = 0; j < AUTOCALIBRATION_SAMPLE_SIZE; j++) {
            sum_SMA_down[i] += user_config_ptr->down_positions[i];
            buf_SMA_down[i][j] = user_config_ptr->down_positions[i];
